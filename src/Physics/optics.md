...
f-number 是一个比值（分数）：这就是为什么它通常写作 f/2.0、f/4.0。其中的 “/” 就是除号。f / 4.0 的意思就是 “焦距除以 4”。

进光量取决于相对孔径（即 f-number），而不是绝对孔径大小。
```
f-number = 焦距（F） / 孔径直径（D）
孔径直径（D） = 焦距（F） / f-number（N）
```
光圈值每变化一档（如 f/2.0 → f/2.8），进光量变为原来的一半（或一倍）。这是因为圆面积与直径的平方成正比。直径变为原来的 1/√2 ≈ 0.7 倍，面积就会变为原来的一半。因此，光圈数值序列是一个公比为 √2 (约 1.414) 的等比数列：
...
用这把“光刀”的光刻设备，基本只能刻出20nm以上制程的芯片。
在晶圆表面和透镜之间，加上一层超纯水，193nm的深紫外光，在水中的折射率为1.44，波长可以进一步缩短到134nm。
浸没式光刻能把制造水平从28nm制程提升到22nm制程。
多重曝光，有LELE工艺，LFLE工艺，SAPD工艺，本质上都是多刻几次的办法。
保证多层刻蚀叠在一起时，能够吻合的误差值，就是“套刻精度”。
考虑成本和效率，制造7nm芯片也几乎是DUV光刻设备的天花板了。

EUV，也是一种光的名字：Extreme Ultra-violet（极紫外光）。波长可以短到只有13.5nm。
这把刀子可以刻出7nm，甚至，5nm，3nm这样更先进的芯片。
EUV光刻机所需的光源、聚焦反射镜、超洁净空间等都是非常尖端的技术。

“华为的战略是从可获取的制造工艺出发，进行系统性的创新与改进”。