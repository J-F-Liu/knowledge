...
> 规范场论的核心思想是：物理系统的拉格朗日量在某种局部对称变换下保持不变，为了保持这种不变性，必须引入一个 “规范场” 作为传递相互力的媒介。
...
杨 - 米尔斯规范场是内部对称群 (U (1), SU (2), SU (3))，作用在 “内部空间”（如电荷、色荷）。

## 时空场(引力场就是时空几何本身)

广义相对论的基础是微分同胚不变性（或等价地说，广义协变性）。它要求物理定律在所有坐标系下形式相同。更进一步，它可以被理解为局域洛伦兹对称性的规范理论。

时空对称群是局域洛伦兹群或庞加莱群，作用在时空本身或局部惯性系。

广义相对论可以被视为以局域洛伦兹群（庞加莱群的齐次部分）为规范群的规范理论。自旋联络是规范势，曲率是场强。

所有能量 - 动量都作为引力源的 “荷”。这意味着引力场与自身的能量 - 动量也耦合，导致非线性。

在量子场论微扰处理中不可重整化，是量子引力面临的核心难题之一。
