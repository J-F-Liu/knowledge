...
读取时没有外部电源向浮栅注入电子。长期频繁读取某些页，可能导致邻近页的电荷发生偏移，最终造成位翻转（bit error）。

如果只是 “读”，电荷不会增加；当 ECC 发现错误时，主控可能主动重写数据页。
只要是完整读取一个文件，ECC 就会被自动触发。如果 “读 + 纠错 + 重写”，电荷会被重新建立。
消费级 U 盘 / 移动 SSD 通常不会主动重写，只有企业级 / 工业级 SSD 或高端主控（如三星、慧荣 SM2262）才支持自动后台刷新。
对于普通 U 盘 / 移动 SSD（如 SanDisk、Kingston），最可靠的方法是每 6~12 个月完整读取一次；每 3~5 年复制出来再写回去（人工刷新）。
...
网线根据其等级、长度不同，能够跑到的最高网速、稳定度也是不同的。
高品质网线，达到万兆（10Gbps）的速率的传输速度长度限制如下：
超五类：45米
六类线：55 米
超六类线: 100 米
7类线：100米以上
...
HDMI 2.0（2013）：18Gbps  4K + 60帧 + HDR10 
HDMI 2.1（2017）：48Gbps  8K + 60帧
HDMI 2.2（2025）：96Gbps  12K + 60帧

坐在距离3.6米的地方，观看 12K 视频，屏幕可以大到550英寸，也能达到视网膜分辨率。
550英寸相当于宽12米，高6.8米，也就是广告牌大小！

GPMI（通用多媒体接口） 支持双向多流、双向控制功能，在菊花链组网技术支持下，可实现多设备 “一线串联”。
GPMI 标准由深圳 8K 超高清视频产业合作联盟牵头，联合华为、海信、创维、华为、海信、创维、 它是与包括 TCL 在内的 50 多家中国公司合作开发的新接口标准。
GPMI Type - B 版本支持192Gbps 的带宽， 480W（智能功率分配）的供电能力，满足 8K@60Hz 无损视频传输，适用于专业影音制作。
GPMI Type - C 版本与现有 USB Type - C 接口兼容，最高可支持 96Gbps 的数据传输和 240W （支持 PD 协议）的电力传输。
